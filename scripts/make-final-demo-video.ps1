...
$frames = Join-Path $out "frames-1080p"
$voiceDir = Join-Path $out "voice"
$ffmpeg = "D:\tools\bin\ffmpeg.exe"
$ffprobe = "D:\tools\bin\ffprobe.exe"

New-Item -ItemType Directory -Force -Path $out, $frames, $voiceDir | Out-Null
...
function To-FfmpegPath($path) {
  return $path.Replace("\", "/")
}

function Normalize-Frame($source, $dest) {
  & $ffmpeg -hide_banner -loglevel error -y -i $source `
    -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0xF5F8FF,setsar=1" `
    -frames:v 1 $dest
}

$visualPlan = @(
  @{ image="00-hero.png"; duration=5.0 },
  @{ image="01-wallet-bubble.png"; duration=2.0 },
  @{ image="01-wallet.png"; duration=7.5 },
  @{ image="01-wallet-click.png"; duration=0.5 },
  @{ image="02-stake-entry-bubble.png"; duration=2.0 },
  @{ image="02-stake-entry.png"; duration=7.5 },
  @{ image="02-stake-entry-click.png"; duration=0.5 },
  @{ image="03-puffer-quote-bubble.png"; duration=2.0 },
  @{ image="03-puffer-quote.png"; duration=10.5 },
  @{ image="03-puffer-quote-click.png"; duration=0.5 },
  @{ image="04-confirmation-bubble.png"; duration=2.0 },
  @{ image="04-confirmation.png"; duration=11.5 },
  @{ image="04-confirmation-click.png"; duration=0.5 },
  @{ image="05-risk-bubble.png"; duration=2.0 },
  @{ image="05-risk.png"; duration=10.5 },
  @{ image="05-risk-click.png"; duration=0.5 },
  @{ image="06-token-core-bubble.png"; duration=2.0 },
  @{ image="06-token-core-start.png"; duration=2.0 },
  @{ image="06-token-core-result.png"; duration=13.0 },
  @{ image="07-complete-bubble.png"; duration=2.0 },
  @{ image="07-complete.png"; duration=12.0 },
  @{ image="08-runtime-puffer-api.png"; duration=4.5 },
  @{ image="08-runtime-token-core.png"; duration=5.0 },
  @{ image="08-runtime-broadcast-gate.png"; duration=4.5 },
  @{ image="09-ending.png"; duration=8.0 }
)

$mainPlan = $visualPlan[0..20]
$runtimePlan = $visualPlan[21..23]

for ($i = 0; $i -lt $visualPlan.Count; $i++) {
  $src = Join-Path $screens $visualPlan[$i].image
  if (!(Test-Path $src)) { throw "Missing screenshot: $src" }
  $dest = Join-Path $frames ("{0:D2}-{1}" -f $i, $visualPlan[$i].image)
  Normalize-Frame $src $dest
  $visualPlan[$i].frame = $dest
}

function Write-ConcatList($path, $plan) {
  $lines = New-Object System.Collections.Generic.List[string]
  foreach ($item in $plan) {
    $lines.Add("file '" + (To-FfmpegPath $item.frame) + "'")
    $lines.Add("duration " + ([string]::Format([System.Globalization.CultureInfo]::InvariantCulture, "{0:0.###}", $item.duration)))
  }
  $last = $plan[$plan.Count - 1]
  $lines.Add("file '" + (To-FfmpegPath $last.frame) + "'")
  Write-Utf8NoBom $path ($lines -join "`n")
}

$fullList = Join-Path $out "visual-full-list.txt"
$mainList = Join-Path $out "main-flow-list.txt"
$runtimeList = Join-Path $out "runtime-evidence-list.txt"
Write-ConcatList $fullList $visualPlan
Write-ConcatList $mainList $mainPlan
Write-ConcatList $runtimeList $runtimePlan

& $ffmpeg -hide_banner -loglevel error -y -f concat -safe 0 -i $mainList -vf "fps=30,format=yuv420p" -c:v libx264 -crf 18 -preset medium (Join-Path $out "raw-main-flow.mp4")
& $ffmpeg -hide_banner -loglevel error -y -f concat -safe 0 -i $runtimeList -vf "fps=30,format=yuv420p" -c:v libx264 -crf 18 -preset medium (Join-Path $out "raw-runtime-evidence.mp4")
& $ffmpeg -hide_banner -loglevel error -y -f concat -safe 0 -i $fullList -vf "fps=30,format=yuv420p" -c:v libx264 -crf 18 -preset medium (Join-Path $out "raw-visual-full.mp4")
...
& $ffmpeg -hide_banner -loglevel error -y @voiceInputs -filter_complex $voiceComplex -map "[voice]" -c:a pcm_s16le (Join-Path $out "voiceover.wav")

& $ffmpeg -hide_banner -loglevel error -y -f lavfi -i "sine=frequency=220:duration=118:sample_rate=48000" -f lavfi -i "sine=frequency=330:duration=118:sample_rate=48000" `
  -filter_complex "[0:a]volume=0.025[a0];[1:a]volume=0.018[a1];[a0][a1]amix=inputs=2:duration=first,afade=t=in:st=0:d=3,afade=t=out:st=114:d=4" `
  -c:a pcm_s16le (Join-Path $out "bgm.wav")

& $ffmpeg -hide_banner -loglevel error -y -f lavfi -i "sine=frequency=980:duration=0.07:sample_rate=48000" `
  -af "volume=0.35,afade=t=out:st=0.025:d=0.045" -c:a pcm_s16le (Join-Path $out "tap.wav")
...
& $ffmpeg -hide_banner -loglevel error -y @tapInputs -filter_complex $tapComplex -map "[taps]" -c:a pcm_s16le (Join-Path $out "clicks.wav")
...
& $ffmpeg -hide_banner -loglevel error -y -i (Join-Path $out "raw-visual-full.mp4") -i (Join-Path $out "voiceover.wav") -i (Join-Path $out "bgm.wav") -i (Join-Path $out "clicks.wav") `
  -filter_complex "[1:a]volume=1.0[a1];[2:a]volume=0.12[a2];[3:a]volume=0.22[a3];[a1][a2][a3]amix=inputs=3:duration=longest:normalize=0[a]" `
  -vf "ass='$subtitlePath'" -map 0:v -map "[a]" -t 118 -c:v libx264 -crf 18 -preset medium -c:a aac -b:a 192k -movflags +faststart $final
...
Write-Host "Final video duration: $duration"
